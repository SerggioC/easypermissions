    ...
    fun onRequestPermissionsResult(
        ...
        receivers.forEach {
            ...
            if (granted.isNotEmpty()) {
                if (it is PermissionCallbacks) {
                    it.onPermissionsGranted(requestCode, granted)
                }
            }
            ...
            if (denied.isNotEmpty()) {
                if (it is PermissionCallbacks) {
                    it.onPermissionsDenied(requestCode, denied)
                }
            }

            // If 100% successful, call annotated methods
            if (granted.isNotEmpty() && denied.isEmpty()) {
                runAnnotatedMethods(it, requestCode)
            }
        }
    }
    ...
    private fun runAnnotatedMethods(`object`: Any, requestCode: Int) {
        ...
        while (clazz != null) {
            for (method in clazz.declaredMethods) {
                val ann =
                    method.getAnnotation<AfterPermissionGranted>(AfterPermissionGranted::class.java)
